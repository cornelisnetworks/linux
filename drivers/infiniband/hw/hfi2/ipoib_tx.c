...
static struct ipoib_txreq *hfi2_txreq_from_idx(struct hfi2_ipoib_circ_buf *r,
					       u32 idx)
{
					...
					struct ipoib_txparms *txp)
{
	...
	for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
		...
		ret = sdma_txadd_page(dd, txreq, skb_frag_page(frag),
				    ...
				    struct ipoib_txparms *txp)
{
	...
	/* add pbc + headers */
	ret = sdma_txadd_kvaddr(dd, txreq, sdma_hdr,
					   ...
					   struct ipoib_txparms *txp)
{
	...
	struct send_context *sc =
		hfi2_qp_to_send_context(priv->qp, txp->flow.sc5);
	struct hfi2_sdma_header *sdma_hdr = tx->sdma_hdr;
	struct sk_buff *skb = tx->skb;
	...
	if (rdma_ah_get_ah_flags(ah_attr) & IB_AH_GRH) {
		...
		txp->hdr_dwords += hfi2_make_grh(
			txp->ibp, grh, rdma_ah_read_grh(ah_attr),
			txp->hdr_dwords - LRH_9B_DWORDS, payload_dwords);
		lrh0 = HFI2_LRH_GRH;
	...
	ohdr->bth[2] =
		cpu_to_be32(mask_psn((u32)txp->txq->tx_ring.sent_txreqs));

	/* Build the deth */
	ohdr->u.ud.deth[0] = cpu_to_be32(priv->qkey);
	ohdr->u.ud.deth[1] =
		cpu_to_be32((txp->entropy << HFI2_IPOIB_ENTROPY_SHIFT) | sqpn);
						      ...
						      struct ipoib_txparms *txp)
{
	...
	if (likely(!ret)) {
		if (txq->flow.as_int != txp->flow.as_int) {
			...
			txq->sde = hfi2_sdma_select_engine_sc(
				ppd, txp->flow.tx_queue, txp->flow.sc5);
				     ...
				     struct hfi2_ipoib_txq *txq)
{
	...
	ret = hfi2_sdma_send_txlist(txq->sde, iowait_get_ib_work(&txq->wait),
				    &txq->tx_list, &count_out);
				...
				struct ipoib_txreq *tx)
{
	...
	ret = hfi2_sdma_send_txreq(txq->sde, iowait_get_ib_work(&txq->wait),
				   &tx->txreq, txq->pkts_sent);
	if (likely(!ret)) {
		txq->pkts_sent = true;
				      ...
				      struct ipoib_txparms *txp)
{
	...
	trace_sdma_output_ibhdr(txq->priv->dd, &tx->sdma_hdr->hdr,
...
static int hfi2_ipoib_send_dma_list(struct net_device *dev, struct sk_buff *skb,
				    struct ipoib_txparms *txp)
{
	struct hfi2_ipoib_txq *txq = txp->txq;
	struct hfi2_ipoib_circ_buf *tx_ring;
...
int hfi2_ipoib_send(struct net_device *dev, struct sk_buff *skb,
		    struct ib_ah *address, u32 dqpn)
{
	...
	if (unlikely(skb->len > rn->mtu + HFI2_IPOIB_ENCAP_LEN)) {
		dd_dev_warn(priv->dd,
			    ...
			    skb->len, rn->mtu + HFI2_IPOIB_ENCAP_LEN);
		++dev->stats.tx_dropped;
		++dev->stats.tx_errors;
		dev_kfree_skb_any(skb);
		return NETDEV_TX_OK;
...
int hfi2_ipoib_txreq_init(struct hfi2_ipoib_dev_priv *priv)
{
	...
	priv->txqs = kcalloc_node(dev->num_tx_queues,
				  sizeof(struct hfi2_ipoib_txq), GFP_KERNEL,
				  priv->dd->node);
	if (!priv->txqs)
		return -ENOMEM;
	...
	for (i = 0; i < dev->num_tx_queues; i++) {
		...
		tx_ring = &txq->tx_ring;
		hfi2_iowait_init(&txq->wait, 0, hfi2_ipoib_flush_txq, NULL,
				 hfi2_ipoib_sdma_sleep, hfi2_ipoib_sdma_wakeup,
				 NULL, NULL);
		txq->priv = priv;
		txq->sde = NULL;
		...
		for (j = 0; j < tx_ring_size; j++) {
			hfi2_txreq_from_idx(tx_ring, j)->sdma_hdr =
				kzalloc_node(sizeof(*tx->sdma_hdr), GFP_KERNEL,
					     priv->dd->node);
...
void hfi2_ipoib_tx_timeout(struct net_device *dev, unsigned int q)
{
	...
	dd_dev_info(
		...
		txq, q, __netif_subqueue_stopped(dev, txq->q_idx),
